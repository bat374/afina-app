...
function Accounts({ accounts, onAdd, onImport, onEdit, debts, onAddDebt, onOpenDebt, currencySettings, onCurrencySettings }: { accounts: Account[]; onAdd: () => void; onImport: () => void; onEdit: (account: Account) => void; debts: Debt[]; onAddDebt: () => void; onOpenDebt: (debt: Debt) => void; currencySettings: CurrencySettings; onCurrencySettings: () => void }) {
  ...
  const [collapsed, setCollapsed] = useState<Partial<Record<AccountType, boolean>>>({}); const [archiveOpen, setArchiveOpen] = useState(false);
  const consolidated = consolidatedNetWorth(accounts, debts, currencySettings);
  ...
  return <ScrollView contentContainerStyle={s.page} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    {!!archivedDebts.length && <><Pressable style={s.groupHeader} onPress={() => setArchiveOpen((value) => !value)}><View><Text style={s.sectionTitle}>Архив долгов</Text><Text style={s.rowSub}>{archivedDebts.length} погашено</Text></View><Ionicons name={archiveOpen ? 'chevron-up' : 'chevron-down'} size={20} color={C.blue} /></Pressable>{archiveOpen && <View style={s.card}>{archivedDebts.map((debt, index) => <Pressable key={debt.id} onPress={() => onOpenDebt(debt)}><View style={s.personRow}><View style={[s.personIcon, { backgroundColor: C.sageSoft }]}><Ionicons name="checkmark" size={18} color={C.green} /></View><View style={{ flex: 1 }}><Text style={s.rowTitle}>{debt.person} · {debt.title}</Text><Text style={s.rowSub}>Погашен · открыть историю</Text></View><Text style={s.rowSub}>{debt.currency}</Text></View>{index < archivedDebts.length - 1 && <View style={s.divider} />}</Pressable>)}</View>}</>}
    <View style={{ height: 20 }} />
  </ScrollView>;
}
...
function Analytics({ accounts, plannedExpenses, debts, currencySettings, operations, userBudgets, financialGoals, onAddBudget, onEditBudget, onAddGoal, onEditGoal }: { accounts: Account[]; plannedExpenses: PlannedExpense[]; debts: Debt[]; currencySettings: CurrencySettings; operations: FinancialOperation[]; userBudgets: Budget[]; financialGoals: FinancialGoal[]; onAddBudget: () => void; onEditBudget: (budget: Budget) => void; onAddGoal: () => void; onEditGoal: (goal: FinancialGoal) => void }) {
  ...
  const spentForBudget = (budget: Budget) => operations.filter((operation) => operation.status !== 'reversed' && !operation.relatedOperationId && operation.kind === 'expense' && operation.currency === budget.currency && operation.category === budget.category && operation.date >= range.from && operation.date <= range.to).reduce((sum, operation) => sum + operation.amount, 0);
  return <ScrollView contentContainerStyle={s.page} showsVerticalScrollIndicator={false}>
...
function DebtEditor({ visible, debt, history, accounts, onClose, onCreate, onUpdate, onPayment, onReversePayment, onExtend, onOverdue }: {
  visible: boolean; debt: Debt | null; history: DebtHistory[]; accounts: Account[]; onClose: () => void;
  onCreate: (input: DebtInput) => Promise<void>; onUpdate: (input: DebtInput) => Promise<void>; onPayment: (amount: number, date: string, accountId?: string, note?: string) => Promise<void>;
  ...
  useEffect(() => {
    ...
    setPaymentAmount(undefined); setPaymentDate(today); setPaymentAccountId(debt?.accountId ?? accounts[0]?.id); setNewDueDate(debt?.dueDate ?? ''); setActionNote('');
  }, [visible, debt]);
...
function AppContent() {
  ...
  const handleDebtPayment = async (amount: number, date: string, accountId?: string, note?: string) => {
    if (!editingDebt) return;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) { Alert.alert('Проверьте дату погашения'); return; }
    const execute = async () => { try { await recordDebtPayment(editingDebt.id, amount, date, accountId ?? null, note); await refreshOpenDebt(); } catch (error) { Alert.alert('Не удалось записать погашение', error instanceof Error ? error.message : 'Проверьте валютные курсы.'); } };
  ...
  return <SafeAreaView style={s.safe} edges={['top']}>
    ...
    <DebtEditor visible={debtEditorOpen} debt={editingDebt} history={debtHistory} accounts={userAccounts} onClose={() => setDebtEditorOpen(false)} onCreate={handleCreateDebt} onUpdate={handleUpdateDebt} onPayment={handleDebtPayment} onReversePayment={handleReverseDebtPayment} onExtend={handleDebtExtension} onOverdue={handleDebtOverdue} />
