...
const debtHistoryLabel = (event: DebtHistory) => ({ created: 'Долг создан', edited: 'Условия изменены', payment: 'Погашение', early_payment: 'Досрочное погашение', extension: 'Пролонгация', overdue: 'Просрочка' }[event.type]);
...
function Home({ onImport, go, accounts, plannedExpenses, debts, currencySettings, onCurrencySettings }: { onImport: () => void; go: (tab: Tab) => void; accounts: Account[]; plannedExpenses: PlannedExpense[]; debts: Debt[]; currencySettings: CurrencySettings; onCurrencySettings: () => void }) {
  ...
  const currencies = Array.from(new Set([...Object.keys(totals), ...plannedExpenses.map((item) => item.currency), ...debts.map((item) => item.currency)])).sort((a, b) => a === 'UZS' ? -1 : b === 'UZS' ? 1 : a.localeCompare(b));
...
function Accounts({ accounts, onAdd, onImport, onEdit, debts, onAddDebt, onOpenDebt, currencySettings, onCurrencySettings }: { accounts: Account[]; onAdd: () => void; onImport: () => void; onEdit: (account: Account) => void; debts: Debt[]; onAddDebt: () => void; onOpenDebt: (debt: Debt) => void; currencySettings: CurrencySettings; onCurrencySettings: () => void }) {
  ...
  const debtGroups = Object.entries(debts.reduce<Record<string, Debt[]>>((result, debt) => ({ ...result, [debt.person]: [...(result[debt.person] ?? []), debt] }), {})).sort(([left], [right]) => left.localeCompare(right, 'ru'));
...
function Calendar({ accounts, plannedExpenses, debts, currencySettings, onAddExpense, onEditExpense }: { accounts: Account[]; plannedExpenses: PlannedExpense[]; debts: Debt[]; currencySettings: CurrencySettings; onAddExpense: (date: string) => void; onEditExpense: (expense: PlannedExpense) => void }) {
  ...
  return <ScrollView contentContainerStyle={s.page} showsVerticalScrollIndicator={false}>
    <Header eyebrow={`СЕГОДНЯ · ${today} · АФИНА 1.0`} title="Платёжный календарь" />
...
function Operations({ operations, accounts, onAdd }: { operations: FinancialOperation[]; accounts: Account[]; onAdd: () => void }) {
  ...
  return <ScrollView contentContainerStyle={s.page} showsVerticalScrollIndicator={false}><Header eyebrow="ДВИЖЕНИЕ ДЕНЕГ" title="Операции" /><Pressable style={s.planExpenseButton} onPress={onAdd}><Ionicons name="add-circle-outline" size={20} color="white" /><Text style={s.importAccountText}>Добавить операцию</Text></Pressable>
    ...
    <SectionTitle title={`Найдено: ${filtered.length}`} />{filtered.length ? <View style={s.card}>{filtered.map((operation, index) => { const account = accounts.find((item) => item.id === operation.accountId); return <View key={operation.id}><View style={s.eventRow}><View style={[s.roundIcon, { backgroundColor: operation.kind === 'income' ? C.sageSoft : C.redSoft }]}><Ionicons name={operation.kind === 'income' ? 'arrow-down-outline' : 'arrow-up-outline'} size={19} color={operation.kind === 'income' ? C.green : C.red} /></View><View style={{ flex: 1 }}><Text style={s.rowTitle}>{operation.title}</Text><Text style={s.rowSub}>{operation.date} · {operation.category} · {account?.name ?? 'Счёт удалён'}</Text></View><Text style={operation.kind === 'income' ? s.income : s.expense}>{operation.kind === 'income' ? '+' : '−'}{money(operation.amount, false, operation.currency)}</Text></View>{index < filtered.length - 1 && <View style={s.divider} />}</View>; })}</View> : <View style={s.emptyCard}><Ionicons name="receipt-outline" size={24} color={C.blue} /><Text style={s.emptyTitle}>Операции не найдены</Text><Text style={s.emptyText}>Измените фильтры или добавьте новую операцию</Text></View>}
...
function Analytics({ accounts, plannedExpenses, debts, currencySettings, operations, userBudgets, financialGoals, onAddBudget, onEditBudget, onAddGoal, onEditGoal }: { accounts: Account[]; plannedExpenses: PlannedExpense[]; debts: Debt[]; currencySettings: CurrencySettings; operations: FinancialOperation[]; userBudgets: Budget[]; financialGoals: FinancialGoal[]; onAddBudget: () => void; onEditBudget: (budget: Budget) => void; onAddGoal: () => void; onEditGoal: (goal: FinancialGoal) => void }) {
  ...
  const byAccount = projection.events.filter((event) => event.kind === 'interest').reduce<Record<string, number>>((result, event) => ({ ...result, [event.accountId]: (result[event.accountId] ?? 0) + event.amount }), {});
  const monthPrefix = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  const visibleBudgets = userBudgets.filter((budget) => budget.currency === currency);
  const spentForBudget = (budget: Budget) => operations.filter((operation) => operation.kind === 'expense' && operation.currency === budget.currency && operation.category === budget.category && operation.date.startsWith(monthPrefix)).reduce((sum, operation) => sum + operation.amount, 0);
  return <ScrollView contentContainerStyle={s.page} showsVerticalScrollIndicator={false}>
    <Header eyebrow={now.toLocaleString('ru-RU', { month: 'long', year: 'numeric' }).toUpperCase()} title="Аналитика" />
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.currencyRail}>{(currencies.length ? currencies : ['UZS']).map((item) => <Pressable key={item} style={[s.currencyFilter, currency === item && s.currencyFilterActive]} onPress={() => setCurrency(item)}><Text style={[s.currencyFilterText, currency === item && s.currencyFilterTextActive]}>{item}</Text></Pressable>)}</ScrollView>
    ...
    <View style={s.metricRow}><View style={s.metric}><Text style={s.metricLabel}>ПАССИВНО</Text><Text style={s.metricValue}>{money(projection.passiveIncome, true, currency)}</Text><Text style={s.metricSub}>проценты</Text></View><View style={s.metric}><Text style={s.metricLabel}>ПЛАН ДОХОДОВ</Text><Text style={s.metricValue}>{money(projection.plannedIncome, true, currency)}</Text><Text style={s.metricSub}>за месяц</Text></View><View style={s.metric}><Text style={s.metricLabel}>ПЛАН РАСХОДОВ</Text><Text style={s.metricValue}>{money(projection.plannedExpense, true, currency)}</Text><Text style={s.metricSub}>за месяц</Text></View></View>
    ...
    {financialGoals.length ? financialGoals.map((goal) => { const goalProjection = buildMonthProjection(accounts, goal.currency, now.getFullYear(), now.getMonth(), plannedExpenses, debts); const progress = calculateGoalProgress(goal, accounts, operations, debts, goalProjection.passiveIncome, now); const color = progress.overdue ? C.red : progress.percent >= 100 ? C.green : C.blue; const typeLabel = goal.type === 'balance' ? 'Остаток' : goal.type === 'monthly_income' ? 'Доход в месяц' : 'Погашение долга'; return <Pressable key={goal.id} style={s.goalCard} onPress={() => onEditGoal(goal)}><View style={s.goalTop}><View style={[s.goalIcon, { backgroundColor: `${color}18` }]}><Ionicons name={goal.type === 'debt_payoff' ? 'checkmark-circle-outline' : goal.type === 'monthly_income' ? 'trending-up-outline' : 'flag-outline'} size={22} color={color} /></View><View style={{ flex: 1 }}><Text style={s.rowTitle}>{goal.title}</Text><Text style={[s.rowSub, progress.overdue && { color: C.red }]}>{typeLabel} · до {goal.deadline}{progress.overdue ? ' · срок истёк' : ''}</Text></View><Text style={[s.goalPercent, { color }]}>{Math.round(progress.percent)}%</Text></View><Progress value={progress.percent} color={color} /><View style={s.goalAmounts}><Text style={s.rowSub}>{money(progress.current, true, goal.currency)}</Text><Text style={s.rowSub}>{money(goal.target, true, goal.currency)}</Text></View></Pressable>; }) : <Pressable style={s.emptyCard} onPress={onAddGoal}><Ionicons name="flag-outline" size={24} color={C.blue} /><Text style={s.emptyTitle}>Финансовых целей пока нет</Text><Text style={s.emptyText}>Создайте цель по остатку, доходу или погашению долга</Text></Pressable>}
    <SectionTitle title="План расходов" />
    {projection.plannedExpense > 0 ? <View style={s.chartCard}><Text style={s.chartLabel}>ЗАПЛАНИРОВАНО · {currency}</Text><Text style={s.chartValue}>{money(projection.plannedExpense, false, currency)}</Text><Text style={[s.rowSub, { marginTop: 7 }]}>Уже учтено в прогнозном остатке календаря</Text></View> : <View style={s.emptyCard}><Ionicons name="receipt-outline" size={24} color={C.blue} /><Text style={s.emptyTitle}>Расходы ещё не запланированы</Text><Text style={s.emptyText}>Добавьте разовый или повторяющийся платёж в календаре</Text></View>}
    <View style={{ height: 20 }} />
  </ScrollView>;
}
...
function GoalEditor({ visible, goal, accounts, debts, onClose, onSave, onDelete }: { visible: boolean; goal: FinancialGoal | null; accounts: Account[]; debts: Debt[]; onClose: () => void; onSave: (input: FinancialGoalInput) => Promise<void>; onDelete: (goal: FinancialGoal) => void }) {
  ...
  const submit = () => {
    ...
    onSave({ title: title.trim(), type, target, currency, deadline, accountId: type === 'balance' ? accountId : undefined, debtId: type === 'debt_payoff' ? debtId : undefined });
  };
  ...
  return <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}><SafeAreaView style={s.modal} edges={['top', 'bottom']}><View style={s.modalHead}><Pressable onPress={onClose} style={s.close}><Ionicons name="close" size={22} color={C.ink} /></Pressable><Text style={s.modalTitle}>{goal ? 'Изменить цель' : 'Новая цель'}</Text><View style={{ width: 40 }} /></View><KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}><ScrollView contentContainerStyle={s.formBody} keyboardDismissMode="on-drag" automaticallyAdjustKeyboardInsets>
    ...
    {type === 'balance' && <><Text style={s.fieldLabel}>КАКОЙ ОСТАТОК СЧИТАТЬ</Text><View style={s.targetList}><Pressable style={[s.targetAccount, !accountId && s.targetAccountActive]} onPress={() => setAccountId(undefined)}><Text style={s.targetAccountText}>Все счета в {currency}</Text></Pressable>{accounts.filter((account) => account.currency === currency).map((account) => <Pressable key={account.id} style={[s.targetAccount, accountId === account.id && s.targetAccountActive]} onPress={() => setAccountId(account.id)}><Text style={s.targetAccountText}>{account.name}</Text><Text style={s.rowSub}>{money(account.balance, false, currency)}</Text></Pressable>)}</View></>}
...
function DebtEditor({ visible, debt, history, accounts, onClose, onCreate, onUpdate, onPayment, onExtend, onOverdue }: {
  visible: boolean; debt: Debt | null; history: DebtHistory[]; accounts: Account[]; onClose: () => void;
  onCreate: (input: DebtInput) => Promise<void>; onUpdate: (input: DebtInput) => Promise<void>; onPayment: (amount: number, date: string, note?: string) => Promise<void>;
  ...
  useEffect(() => {
    ...
    setPaymentAmount(debt?.currentBalance); setPaymentDate(today); setNewDueDate(debt?.dueDate ?? ''); setActionNote('');
  }, [visible, debt]);
  ...
  const historyRows = history.map((event, index) => <View key={event.id} style={[s.historyRow, index > 0 && s.budgetBorder]}><View style={{ flex: 1 }}><Text style={s.rowTitle}>{debtHistoryLabel(event)}</Text><Text style={s.rowSub}>{event.occurredAt.slice(0, 10)}{event.fromDate && event.toDate ? ` · ${event.fromDate} → ${event.toDate}` : ''}{event.note ? ` · ${event.note}` : ''}</Text></View>{event.amount !== undefined && <Text style={s.budgetAmount}>{money(event.amount, false, debt?.currency ?? currency)}</Text>}</View>);
  return <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}><SafeAreaView style={s.modal} edges={['top', 'bottom']}>
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}><ScrollView contentContainerStyle={s.formBody} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" automaticallyAdjustKeyboardInsets>
      {!debt ? <>
        ...
        {debt.status !== 'paid' && <><Text style={s.fieldLabel}>ПОГАШЕНИЕ</Text><DecimalInput value={paymentAmount} onChange={setPaymentAmount} placeholder="Сумма" /><View style={{ marginTop: 8 }}><DateField value={paymentDate} onPress={() => setDateTarget('payment')} /></View><Pressable style={s.primaryButton} onPress={() => paymentAmount && onPayment(paymentAmount, paymentDate, actionNote || undefined)}><Text style={s.primaryText}>Записать погашение</Text></Pressable>
...
function AppContent() {
  ...
  const handleDebtPayment = async (amount: number, date: string, note?: string) => {
    if (!editingDebt) return;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) { Alert.alert('Проверьте дату погашения'); return; }
    try { await recordDebtPayment(editingDebt.id, amount, date, note); await refreshOpenDebt(); }
    catch (error) { Alert.alert('Не удалось записать погашение', error instanceof Error ? error.message : 'Проверьте валютные курсы.'); }
  };
  ...
  return <SafeAreaView style={s.safe} edges={['top']}>
    ...
    <DebtEditor visible={debtEditorOpen} debt={editingDebt} history={debtHistory} accounts={userAccounts} onClose={() => setDebtEditorOpen(false)} onCreate={handleCreateDebt} onUpdate={handleUpdateDebt} onPayment={handleDebtPayment} onExtend={handleDebtExtension} onOverdue={handleDebtOverdue} />
